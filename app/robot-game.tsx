...
export default function RobotGame({ totalErrors, totalHints, onError, onHint, onComplete, onFinal, onExit }: Props) {
  ...
  const [failures, setFailures] = useState(0); const [hintUsed, setHintUsed] = useState(false); const [duckMessage, setDuckMessage] = useState<RobotDuckMessage>("intro"); const [duckClosing, setDuckClosing] = useState(false); const [finalUnlocked, setFinalUnlocked] = useState(false);
  ...
  function showHint() { if (duckTimer.current !== null) window.clearTimeout(duckTimer.current); duckTimer.current = null; setDuckClosing(false); if (!hintUsed) { setHintUsed(true); onHint(); updateQuestProgress({ robotHintUsed: true }); } setDuckMessage("hint"); }
