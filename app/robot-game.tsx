...
export default function RobotGame({ totalErrors, totalHints, onError, onHint, onComplete, onFinal, onExit }: Props) {
  ...
  const [hintMode, setHintMode] = useState<"closed" | "shown">("closed"); const [introOpen, setIntroOpen] = useState(false); const [finalUnlocked, setFinalUnlocked] = useState(false);
  ...
  function showHint() { setIntroOpen(false); if (!hintUsed) { setHintUsed(true); onHint(); updateQuestProgress({ robotHintUsed: true }); } setHintMode("shown"); }
  ...
  return <QuestStepShell code="ROBOT" step={6} title="Доставь код на сервер" errors={totalErrors} hints={totalHints} onExit={onExit}><section className={`robot-layout robot-program-layout robot-${status}`}>
    ...
    <div className="program-panel robot-program-panel"><span className="game-kicker">ЗАДАНИЕ 06 / ROBOT PROGRAM</span><h1>Напиши код.<br /><em>Запусти утку.</em></h1><p>Самостоятельно напиши программу, которая будет вести утку до сервера. Алгоритм повторяется, пока робот не окажется на цели.</p>
      ...
      <div className="program-actions robot-program-actions">{status === "failed" ? <button className="edit-program" onClick={editProgram}>ИЗМЕНИТЬ АЛГОРИТМ</button> : <button className="run-program" onClick={() => void runProgram()} disabled={locked || !code.trim()}>ЗАПУСТИТЬ <span>↗</span></button>}<button className="precompile-program" onClick={precompile} disabled={locked || !code.trim()}>ПРЕДКОМПИЛЯЦИЯ</button><button className="reset-program" onClick={resetProgram} disabled={locked}>СБРОСИТЬ</button><button className="robot-hint-button" onClick={showHint} disabled={locked}><span>?</span>ПОДСКАЗКА</button></div>
      ...
      <div className={`robot-hint-helper ${hintMode === "shown" || introOpen ? "is-talking" : ""}`}><div className="duck-speech">{introOpen ? <><span>О, настоящий код! Я выполню его буквально. Надеюсь, страховка для клюва предусмотрена.</span><button onClick={() => setIntroOpen(false)}>КОНЕЧНО!</button></> : <><span>Сначала реши, что робот должен делать, пока путь перед ним свободен. Поворот нужен только тогда, когда дальше двигаться прямо нельзя.</span><button onClick={() => setHintMode("closed")}>СПАСИБО!</button></>}</div><RoboDuckFace /></div>
    </div></section>
